...
import { composerPlainText, applyInlineFormat, deleteChipBeforeCaret, deleteSelectionInEditor, insertPlainTextAtCaret, normalizeComposerEditorDom, RICH_INPUT_SLOT } from './rich-editor';
...
export function ChatBar({ busy, cwd, disabled, focusKey, gateway, maxRecordingSeconds = 120, queueSessionKey, sessionId, state, onCancel, onAddUrl, onAttachDroppedItems, onAttachImageBlob, onPasteClipboardImage, onPickFiles, onPickFolders, onPickImages, onRemoveAttachment, onSteer, onSubmit, onTranscribeAudio }) {
    ...
    const applyComposerFormat = (kind) => {
        const editor = editorRef.current;
        if (!editor || inputDisabled) {
            return;
        }
        editor.focus();
        if (!applyInlineFormat(editor, kind)) {
            return;
        }
        flushEditorToDraft(editor);
    };
    const controls = (_jsx(ComposerControls, { autoSpeak: autoSpeak, busy: busy, busyAction: busyAction, canSteer: canSteer, canSubmit: canSubmit, compactModelPill: poppedOut, conversation: {
        ...
        }, disabled: disabled, formatControls: {
