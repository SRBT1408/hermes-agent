...
const INLINE_FORMATS = {
    bold: { placeholder: 'bold', tag: 'strong' },
    italic: { placeholder: 'italic', tag: 'em' },
    code: { placeholder: 'code', tag: 'code' }
};
const ESC = { '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#039;' };
...
function appendInlineFormatting(target, text) {
    ...
    for (const match of text.matchAll(INLINE_FORMAT_RE)) {
        ...
        const code = match[6];
        const value = bold ?? italic ?? code;
        if (value) {
            const el = document.createElement(bold ? 'strong' : italic ? 'em' : 'code');
...
function ensureComposerSelection(editor) {
    const existing = composerSelectionRange(editor);
    if (existing) {
        return existing;
    }
    editor.focus();
