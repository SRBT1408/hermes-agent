...
export function ComposerControls({ autoSpeak, busy, busyAction, canSteer, canSubmit, compactModelPill = false, conversation, disabled, formatControls = null, formattingOpen: controlledFormattingOpen, hasComposerPayload, onFormattingOpenChange, state, voiceStatus, onDictate, onSteer, onToggleAutoSpeak }) {
    const { t } = useI18n();
    const c = t.composer;
    const [internalFormattingOpen, setInternalFormattingOpen] = useState(false);
    const formattingOpen = controlledFormattingOpen ?? internalFormattingOpen;
    const setFormattingOpen = (next) => {
        if (controlledFormattingOpen === undefined) {
            setInternalFormattingOpen(next);
        }
        onFormattingOpenChange?.(next);
    };
    const steerCombo = formatCombo('mod+enter');
    const steerLabel = `${c.steer} (${steerCombo})`;
    const formattingLabel = formattingOpen ? c.hideFormatting : c.showFormatting;
    ...
    return (_jsxs("div", { className: "ml-auto flex shrink-0 items-center gap-(--composer-control-gap)", children: [formatControls ? (_jsxs("div", { className: "mr-1 flex items-center gap-1", children: [_jsx(Tip, { label: formattingLabel, children: _jsx(Button, { "aria-expanded": formattingOpen, "aria-label": formattingLabel, "aria-pressed": formattingOpen, className: cn(GHOST_ICON_BTN, 'gap-0.5 px-1.5 text-[0.78rem] font-medium', formattingOpen && 'bg-accent text-foreground'), disabled: disabled, onClick: () => setFormattingOpen(!formattingOpen), onMouseDown: event => event.preventDefault(), size: "sm", type: "button", variant: "ghost", children: _jsxs("span", { "aria-hidden": true, className: "inline-flex items-baseline gap-0.5", children: [_jsx("span", { className: "font-serif text-[0.9rem] font-semibold leading-none", children: "A" }), _jsx(Codicon, { name: "edit", size: "0.72rem" })] }) }) }), formattingOpen ? _jsx(FormattingToolbar, { controls: formatControls, disabled: disabled, labels: c }) : null] })) : null, _jsx(ModelPill, { compact: compactModelPill, disabled: disabled, model: state.model }), canSteer ? (_jsx(Tip, { label: steerTip, children: _jsx(Button, { "aria-label": steerLabel, className: GHOST_ICON_BTN, disabled: disabled, onClick: onSteer, size: "icon", type: "button", variant: "ghost", children: _jsx(SteeringWheel, { className: iconSize.sm }) }) })) : (_jsx(DictationButton, { disabled: disabled, onToggle: onDictate, state: state.voice, status: voiceStatus })), _jsx(AutoSpeakButton, { active: autoSpeak, disabled: disabled, onToggle: onToggleAutoSpeak }), showVoicePrimary ? (_jsx(Tip, { label: c.startVoice, children: _jsx(Button, { "aria-label": c.startVoice, className: PRIMARY_ICON_BTN, disabled: disabled, onClick: () => {
                        triggerHaptic('open');
...
function FormattingToolbar({ controls, disabled, labels }) {
    return (_jsxs("div", { "aria-label": labels.formattingToolbar, className: "flex items-center gap-1 rounded-lg border border-border/60 bg-background/90 px-1 py-0.5 shadow-sm", role: "toolbar", children: [_jsx(FormattingButton, { ariaLabel: labels.formatBold, className: "font-semibold", disabled: disabled, label: "B", onClick: controls.onBold }), _jsx(FormattingButton, { ariaLabel: labels.formatItalic, className: "italic", disabled: disabled, label: "I", onClick: controls.onItalic }), _jsx(FormattingButton, { ariaLabel: labels.formatCode, className: "font-mono text-[0.7rem]", disabled: disabled, label: "</>", onClick: controls.onCode })] }));
}
function FormattingButton({ ariaLabel, className, disabled, label, onClick }) {
    return (_jsx(Tip, { label: ariaLabel, children: _jsx(Button, { "aria-label": ariaLabel, className: cn('size-7 rounded-md px-0 text-[0.78rem] text-(--ui-text-tertiary) hover:bg-(--chrome-action-hover) hover:text-foreground', className), disabled: disabled, onClick: onClick, onMouseDown: event => event.preventDefault(), type: "button", variant: "ghost", children: _jsx("span", { "aria-hidden": true, children: label }) }) }));
}
